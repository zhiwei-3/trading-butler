...
def _eval_setup_forming(a):
    """Global heads-up evaluator — runs for every strategy, not just smc_confluence."""
    if not ALERT_STATE.get("setup_forming_enabled", True):
        return []

    rsi_val, close_price = a["rsi_val"], a["close_price"]
    ...
    if in_approach_zone:
        ...
        if (valid_bullish_watch or valid_bearish_watch) and ALERT_STATE.get("last_watch_signal") is None:
            ...
            watch_found.append(
                ...
                f"• **Symbol:** `{a.get('symbol', 'XAUUSD')}` | **Price:** `${close_price}`\n"
...
def _eval_ema_cross(a):
    ...
    if bullish_cross and ALERT_STATE["last_rsi_signal"] != "BUY":
        ...
        if targets["rrr"] >= ALERT_STATE.get("min_rrr", 1.0):
            ...
            sl_price, tp1_price, tp2_price = targets["sl_price"], targets["tp1_price"], targets["tp2_price"]
            base_msg = f"🏆 **EMA CROSS SELL ALERT** 🔴\n\n📍 **Entry:** `${close_price}` | 🛡️ **SL:** `${sl_price}` | 🎯 **TP1:** `${tp1_price}`"
            signals.append(_package_signal(a, "SELL", close_price, sl_price, tp1_price, tp2_price, base_msg, score=70))
...
def _eval_rsi_reversion(a):
    ...
    if bullish_rev and ALERT_STATE["last_rsi_signal"] != "BUY":
        ...
        if targets["rrr"] >= ALERT_STATE.get("min_rrr", 1.0):
            ALERT_STATE["last_rsi_signal"] = "SELL"
            sl_price, tp1_price, tp2_price = targets["sl_price"], targets["tp1_price"], targets["tp2_price"]
            base_msg = f"🔄 **RSI REVERSION SELL ALERT** 🔴\n\n📍 **Entry:** `${close_price}` | 🛡️ **SL:** `${sl_price}` | 🎯 **TP1:** `${tp1_price}`"
            signals.append(_package_signal(a, "SELL", close_price, sl_price, tp1_price, tp2_price, base_msg, score=75))
...
def analyze_market(symbol):
    ...
    near_zone = nearest_sr_zone(sr_zones, close_price, max_distance_pct=ALERT_STATE["sr_max_distance_pct"])
...
def evaluate_signals(a):
    ...
    watches = _eval_setup_forming(a)   # independent of active_strategy
    ...
    if strat == "ema_cross":
        signals, _ = _eval_ema_cross(a)
    elif strat == "rsi_reversion":
        signals, _ = _eval_rsi_reversion(a)
    else:
    ...
    if is_oversold and (trend_bullish or macro_bullish) and buy_structure_ok and vol_filter_hard_ok:
        ...
        if score >= min_score and ALERT_STATE.get("last_rsi_signal") != "BUY":
            ...
            if targets["rrr"] >= ALERT_STATE.get("min_rrr", 1.3):
                ...
                signals_found.append(_package_signal(a, "BUY", close_price, sl_price, tp1_price, tp2_price, msg, score=score))
